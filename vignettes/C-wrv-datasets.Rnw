...
Set a \textbf{raster} package option to prevent the standardization of raster names:
...
<<>>=
url.git <- "https://raw.githubusercontent.com/jfisher-usgs/wrv/master/inst/extdata/"
@

The data file containing land-surface elevations was deemed too large in file size (about 500 megabytes) to be placed in the package repository.
...
The common coordinate reference system (CRS) applied to all spatial datasets is the
...
Create a linear regression model between the average of the precipitation depth recorded at the Picabo and Ketchum weather stations,
...
A strong positive correlation (R-squared of \Sexpr{format(summary(LM)$r.squared, digits=3)})
...
Remove negative values of gage height (n = \Sexpr{length(which(d < 0))}):
...
Missing data at the Stanton Crossing near Bellevue gage are replaced with average gage-height values recorded at this gage.
...
During the month of October the water district stops monitoring diversions and much of the water diverted into the Bypass Canal is returned to the Big Wood River at Wood River Ranch.
...
There are a total of \Sexpr{format(nrow(d), big.mark=",")} pumping wells;
...
\subsubsection{Groundwater flow in the tributary canyons (tributaries)}
...
For each tributary canyon, the mean groundwater flow rate is first estimated using a \href{http://en.wikipedia.org/wiki/Darcy_law}{Darcian} analysis.
...
A drainage basin area for a tributary canyon is defined as the area of land where groundwater from rain or melting snow converges at the boundary.
...
Datasets processed at level~2 result from analyses of level~1 data.
...
Calculate rasterized monthly volumetric flow rates across the water table and beneath the non-irrigated lands.
