...
Load the \textbf{wrv} package in the current \R{} session:

<<warning=FALSE, message=FALSE, results="hide">>=
library("wrv")
@

The memory requirement for running \R{} code in this vignette is about 5 gigabytes.
...
The elevation of the pre-Quaternary bedrock surface and top of Quaternary basalt is calculated by subtracting the thickness of the Quaternary sediments from land-surface elevations.
...
The estimated areal extent of the basalt unit in the WRV aquifer system, as defined by \citet[plate~1]{Bartolino2012},
...
Subtracting bedrock-surface elevations from land-surface elevations gives the thickness of the WRV aquifer system (\hyperref[fig:map_aquifer_thickness]{fig.~\ref{fig:map_aquifer_thickness}}).
...
Well-driller reports and geophysical surveys describe the clay confining unit as about 5 m (16 ft) thick, and generally lying at a depth of about 30 m (98 ft) below land surface.
...
The total number of vertically adjusted cells is \Sexpr{format(length(adj.cells), big.mark=",")}, or \Sexpr{round(length(adj.cells) / length(cells) * 100)} percent of active cells,
...
Subtracting the aquitard (confining unit) thickness (5 m [16 ft]) from the bottom of model layer 1 gives the bottom elevation of model layer 2.
...
The bottom elevation of model layer 3 represents the top of the bedrock (bedrock surface).
...
Bottom elevations of model layer 1 are adjusted to the bedrock surface in cells where the layer 1 bottom elevation is above bedrock and the vertically adjacent layer 2 cell is classified as inactive.
...
val <- c(nrow(rs.model), ncol(rs.model), 3, nactive,
         res(rs.model), xmin(rs.model), ymin(rs.model))
...
Stress period identifiers are specified using a concatenation of year and month.
...
\begin{enumerate}[label=Zone \arabic*, topsep=0mm, labelindent=5mm, leftmargin=*]
...
Steady-state volumetric flow rates are calculated for each boundary by averaging flows over time.

<<>>=
flow <- cbind(flow, ss = apply(flow[, ss.yr.mo], 1, mean))
@

The volumetric flow rate for each boundary is uniformly distributed among its tributary cells.
...
The tributary boundary conditions are placed in a single data table.
...
The drain boundary conditions are bundled into a single data table.
...
Surface-water features that are not accounted for by the river subreaches (such as tributary streams, canals, and ponds) are represented in the model as groundwater flow across the water table;
see `\hyperref[sec:recharge]{Groundwater flow across the water table and pumping demand}' section for details.
...
The elevation of the bottom of the riverbed sediments is defined as:
...
The stream-stage elevation ($h_{r}$ in equations~\ref{eq:river} and \ref{eq:rbot}) is initialized at land surface.

<<>>=
rs.model[["riv.stage"]] <- mask(rs.model[["lay1.top"]], rs.model[["riv.reach"]])
@

And lowered if the stream stage in the adjacent upstream river-reach cell is less than the estimated stage of the current downstream cell.
...
Subtracting the average water depth and riverbed thickness from stream stage gives the elevation of the riverbed bottom (equation~\ref{eq:rbot}).
...
Flow between adjacent river cells is ensured by lowering the riverbed bottom elevations in those cells prohibiting vertical connectivity.
...
Subtracting the riverbed top elevation from the stream-stage elevation gives the modeled surface-water depth.
...
@
The modeled surface-water depth ranges from
...
Cell thickness is adjusted to prevent the river stage from ever being below the bottom of a river cell in model layer 1.
...
The northing-distance to the cell center is calculated for each of the river-boundary cells composing the Big Wood River.
...
The modeled stream stage is calculated by subtracting the interpolated normalized mean monthly gage-height from the estimated median stream depth.
...
River boundary conditions are placed in a single table.
...
Steady-state stream-stage elevations are calculated by averaging elevations over time.
...
\subsubsection{Groundwater flow across the water table and pumping demand}
\label{sec:recharge}

Groundwater flow across the water table (that is natural and incidental groundwater recharge and discharge) and pumping demand (that is groundwater diverted from the aquifer by means of pumping wells and flowing-artesian wells)
...
In this study, two water-balance models were developed and used to reconstruct groundwater flow across the water table and pumping demand during the simulation period (1995--2010).
...
Groundwater flow across the water table and pumping demand are specified for each month in the model simulation period
(\hyperref[fig:graph_areal_rech]{figs.~\ref{fig:graph_areal_rech}} and \ref{fig:graph_wells}).
The steady-state groundwater flow across the water table is shown in \hyperref[fig:map_areal_rech]{figure~\ref{fig:map_areal_rech}}.
...
<<include=FALSE>>=
v <- "Total groundwater flow across the water table. Values are preliminary and were modified by adjustments to irrigation efficiency during the model-calibration process."
v <- c(paste("Graph showing", paste0(tolower(substr(v, 1, 1)), substr(v, 2, nchar(v)))), v)
@
...
<<include=FALSE>>=
v <- "Steady-state groundwater flow across the water table. Values are preliminary and were modified by adjustments to irrigation efficiency during the model-calibration process."
v <- c(paste("Map showing", paste0(tolower(substr(v, 1, 1)), substr(v, 2, nchar(v)))), v)
@
...
are known sources of groundwater recharge across the water table that are assigned directly to model cells.
...
Water-table and pumping boundary conditions are placed in a single data table.
...
Steady-state volumetric flow rates are calculated by averaging flows over time.
...
Copy the MODFLOW-USG executable file to the appropriate directory:
...
(1) groundwater flow across the water table, includes incidental and natural groundwater recharge and discharge;
...
rows <- c("Areal recharge", "Streamflow losses", "Tributary fluxes",
...
FUN <- function(desc) {
  ...
  ylab <- paste("Water-table residual, in", c("meters (m)", "feet"))
...
\begin{figure}
  ...
  \end{subfigure}
  <<include=FALSE>>=
  v <- "Water-table residuals in (\\textit{\\textbf{A}}) U.S. Geological Survey (USGS) groundwater-monitoring network wells, (\\textit{\\textbf{B}}) geolocated driller wells, (\\textit{\\textbf{C}}) Public Land Survey System (PLSS)-located driller wells, and (\\textit{\\textbf{D}}) two of the Sun Valley Water and Sewer District (SVWSD) production wells and The Nature Conservancy (TNC) groundwater-monitoring network wells---based on uncalibrated model results. \\label{fig:graph_res_sim}"
...
proj4string(p) <- crs(r)
ids <- sapply(slot(p, "polygons"), function(x) slot(x, "ID"))
d <- data.frame(value=rep(0, length(ids)), row.names=ids)
p <- SpatialPolygonsDataFrame(p, d)
writeOGR(p, dsn=path, layer="domain", driver="ESRI Shapefile",
...
r <- raster(rs.model)
e <- rs.model@extent
pts <- rbind(sw=c(e@xmin, e@ymin), se=c(e@xmax, e@ymin),
             ne=c(e@xmax, e@ymax), nw=c(e@xmin, e@ymax))
corners <- c("south-west", "south-east", "north-east", "north-west")
crs.xy <- crs(r)
pts.xy <- SpatialPoints(pts, proj4string=crs.xy)
x <- coordinates(pts)[, 1]
y <- coordinates(pts)[, 2]
xy <- data.frame(Location=corners, Easting=x, Northing=y, row.names=NULL)
xy <-  paste(knitr::kable(xy, "markdown"), collapse="\n")
...
ll <- paste(knitr::kable(ll, "markdown", digits=6, format.args=list(nsmall=6)), collapse="\n")
replacement <- list(xy=xy, crs.xy=crs.xy@projargs, ll=ll, crs.ll=crs.ll@projargs)
...
idx <- findInterval(80, cumsum(nchar(pkgs)))  # add new line for long string
if (idx < length(pkgs)) pkgs[idx] <- paste("\n         ", pkgs[idx])
...
file <- system.file("misc", "usgsdatarelease.xml", package="wrv")
invisible(file.copy(file, path))

file <- file.path(path, "Thumbnail.png")
...
legend("topleft",
       c("Model extent", "Inactive model domain", "Active model domain"), pch=c(22, 22, 22),
