...
<<graph_areal_rech, echo=FALSE, fig.width=fin.graph.short[1], fig.height=fin.graph.short[2], fig.cap="{Total groundwater flow across the water table.}">>=
d <- TidyBudget(b, "wells", 1L)[, c("totim.date", "flow.total", "flow.total")]
d[d[, 2] < 0, 2] <- 0
d[d[, 3] > 0, 3] <- 0
ylab <- paste("Total flow across the water table, in", c("cubic meters per day", "acre-feet per year"))
col <- c("#67A9CF", "#C80C0B")
...
Descriptive statistics for volumetric recharge rates across the water table, in cubic meters per day:
