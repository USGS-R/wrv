...
\noindent And estimates of surface runoff can be expressed as:
...
\noindent The precipitation-redistribution model assumes that at least 25 percent of a recorded winter-month precipitation is available for immediate deep percolation and surface runoff during the same month.
...
\noindent ET estimates based on the NDVI method are then expressed as:
...
\noindent The METRIC-NDVI method assumes that the ratio $r$ is constant throughout the WRV.
