...
The form for function names is no separator between words and initial capital letters (such as \code{AddScaleBar});
...
% Create Rd2.tex from the command line:
%   R CMD Rd2pdf --no-clean --force "./wrv"
